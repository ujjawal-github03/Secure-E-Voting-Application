...
const UserDashboard = () => {
  ...
  const [votedCandidateInfo, setVotedCandidateInfo] = useState(null);
    ...
    try {
      const response = await fetch(`${API_URL}/review/submit`, {
        ...
        body: JSON.stringify({
          ...
          candidateId: votedCandidateInfo.candidateId,
        }),
      });
      ...
      if (response.ok) {
        alert(`Thank you for your ${data.sentiment} review!`);
        setShowReviewModal(false);
        setVotedCandidateInfo(null);
          ...
          <div className="flex items-center">
            <div
              className={`h-3 w-3 rounded-full mr-3 ${
                userProfile.isVoted ? "bg-green-500" : "bg-red-500"
              }`}
            ></div>
    ...
    const handleVote = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/candidate/vote/${selectedCandidate}`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${getToken()}`,
        ...
        if (response.ok) {
          // Find the candidate name for the review modal
          const candidate = candidates.find((c) => c._id === selectedCandidate);

          // Show review modal after successful vote
          setVotedCandidateInfo({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <ReviewModal
        ...
        candidateName={votedCandidateInfo?.candidateName || ""}
      />
    </div>
  );
};
